...
function GetErrorMessage(err: DropboxTypes.Error<any>): string {
  if (err['user_message']) {
    ...
    throw new Error(
      `Dropbox's servers gave us a garbage error message: ${JSON.stringify(
        err,
      )}`,
    )
  }
    ...
    .catch((e: DropboxTypes.Error<DropboxTypes.files.DeleteError>) => {
      ...
      switch (err['.tag']) {
        case 'path_lookup':
          cb(
            LookupErrorToError(
              (<DropboxTypes.files.DeleteErrorPathLookup>err).path_lookup,
              p,
              GetErrorMessage(e),
            ),
          )
        ...
        case 'path_write':
          cb(
            WriteErrorToError(
              (<DropboxTypes.files.DeleteErrorPathWrite>err).path_write,
              p,
              GetErrorMessage(e),
            ),
          )
        ...
        case 'too_many_write_operations':
          setTimeout(
            () => FilesDeleteWrapped(client, p, cb),
            500 + 300 * Math.random(),
          )
          break
        case 'other':
...
export class DropboxFile extends PreloadFile<DropboxFileSystem>
  implements File {
  constructor(
...
export default class DropboxFileSystem extends BaseFileSystem
  ...
  public static Create(
    opts: DropboxFileSystemOptions,
    cb: BFSCallback<DropboxFileSystem>,
  ): void {
    ...
    this.stat(newPath, false, (e, stats?) => {
      const rename = () => {
        ...
        this._client
          ...
          .then(() => cb())
          .catch(function(
            e: DropboxTypes.Error<DropboxTypes.files.RelocationError>,
          ) {
            const err = ExtractTheFuckingError(e)
            switch (err['.tag']) {
              case 'from_lookup':
                cb(
                  LookupErrorToError(
                    (<DropboxTypes.files.RelocationErrorFromLookup>err)
                      .from_lookup,
                    oldPath,
                    GetErrorMessage(e),
                  ),
              ...
              case 'from_write':
                cb(
                  WriteErrorToError(
                    (<DropboxTypes.files.RelocationErrorFromWrite>err)
                      .from_write,
                    oldPath,
                    GetErrorMessage(e),
                  ),
              ...
              case 'to':
                cb(
                  WriteErrorToError(
                    (<DropboxTypes.files.RelocationErrorTo>err).to,
                    newPath,
                    GetErrorMessage(e),
                  ),
                )
  ...
  public createFile(
    ...
    const blob = new Blob([buffer2ArrayBuffer(fileData) as ArrayBuffer], {
      type: 'octet/stream',
    })
      ...
      .catch((e: DropboxTypes.Error<DropboxTypes.files.UploadError>) => {
        ...
        switch (<string>err['.tag']) {
          ...
          case 'too_many_write_operations':
            // Retry in (500, 800) ms.
            setTimeout(
              () => this.createFile(p, flags, mode, cb),
              500 + 300 * Math.random(),
            )
            break
          case 'other':
    ...
    this.stat(parent, false, (e, stats?) => {
      if (e) {
        ...
        this._client
          ...
          .then(() => cb())
          .catch(
            (e: DropboxTypes.Error<DropboxTypes.files.CreateFolderError>) => {
              const err = ExtractTheFuckingError(e)
              if (<string>err['.tag'] === 'too_many_write_operations') {
                // Retry in a bit.
                setTimeout(
                  () => this.mkdir(p, mode, cb),
                  500 + 300 * Math.random(),
                )
              } else {
                cb(
  ...
  public _syncFile(p: string, d: Buffer, cb: BFSOneArgCallback): void {
    const blob = new Blob([buffer2ArrayBuffer(d) as ArrayBuffer], {
      type: 'octet/stream',
    })
...
function ProcessListFolderError(
  e: DropboxTypes.Error<DropboxTypes.files.ListFolderError>,
  path: string,
...
function ContinueReadingDir(
  ...
  const newEntries = <string[]>(
    res.entries.map(e => e.path_display).filter(Boolean)
  )
